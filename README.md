...
3. **`Content_Scraper`** (`src/Action/Content_Scraper.php`) — fetches a URL (following redirects) and exposes the raw HTML.
...
8. **`Import_Command`** (`src/Command/Import_Command.php`) — the `wp scraper-to-wp` command that drives all of the above.
...
```php
class My_CSV_Provider extends \A8C\SpecialProjects\ScraperToWP\Provider\CSV_URL_Provider {
...
Add a class under `src/Mapper/` in the plugin's namespace. Extend `Default_Content_Mapper` and override only what you need. Read the raw HTML with `$this->content_scraper->get_content()`.
...
use A8C\SpecialProjects\ScraperToWP\Provider\My_URL_Provider;
use A8C\SpecialProjects\ScraperToWP\Mapper\My_Mapper;
...
| `set_content_mapper( string $class )` | `get_content_mapper( Content_Scraper ): Abstract_Content_Mapper` | `Default_Content_Mapper` | The HTML→post mapper (by class name). |
...
```bash
wp scraper-to-wp [--dry-run] [--delay=<seconds>] [--per=<number>] [--silent] [--reset-progress]
```
...
│   ├── Command/Import_Command.php   # `wp scraper-to-wp`
│   ├── Provider/
...
│   ├── Action/
│   │   ├── Content_Scraper.php     # fetch + follow redirects
