...
class Progress_Tracker {
	...
	public function add_scraped_url( string $url ): void {
		...
		// Generate HTTP and HTTP versions of the URL for comparison, BUT ONLY AT THE START,, url may contain https or http
		$http_url = preg_replace( '/^https:/i', 'http:', $url );
		$https_url = preg_replace( '/^http:/i', 'https:', $url );

		if ( ! in_array( $http_url, $this->scraped_urls, true ) ) {
			$this->scraped_urls[] = sanitize_text_field( $http_url );
			$this->save_progress();
		}

		if ( ! in_array( $https_url, $this->scraped_urls, true ) ) {
			$this->scraped_urls[] = sanitize_text_field( $https_url );
			$this->save_progress();
		}
	}
